...
import { ChevronLeft, ChevronRight, Sparkles, Zap, RefreshCw } from "lucide-react"; // Removed RotateCcw, Eye
...
export function CriticalMoveViewer({ playerData }: CriticalMoveViewerProps) {
  ...
  const goToPrevious = () => {
    setCurrentIndex((prev) => (prev > 0 ? prev - 1 : Math.max(0, allHighlightedMoves.length - 1)));
  };
  ...
  if (!currentMove || allHighlightedMoves.length === 0) {
    // ... No moves found card ...
    return (
    ...
    <Card className="bg-gray-900 border-gray-800">
      <CardHeader>
        <div className="flex justify-between items-start flex-wrap gap-2">
          <div>
             <CardTitle>Critical Moves Review</CardTitle>
             <CardDescription>
                Move {currentIndex + 1} of {allHighlightedMoves.length} ({isMoveMade ? "Position After Your Move" : "Position Before - Make the Move!"})
             </CardDescription>
             {/* ... game link ... */}
      ...
      <CardContent className="flex flex-col items-center space-y-4">
        ...
        <div className="flex justify-center items-center space-x-4 w-full pt-2">
          <Button onClick={goToPrevious} variant="outline" size="icon" disabled={allHighlightedMoves.length <= 1} title="Previous Move">
          ...
          <Button onClick={goToNext} variant="outline" size="icon" disabled={allHighlightedMoves.length <= 1} title="Next Move">
